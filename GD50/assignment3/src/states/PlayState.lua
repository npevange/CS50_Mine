    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                :finish(function()
                    keepswap = self:calculateMatches()
                    if keepswap == false and self.highlightedTile ~= nil then
                        local swapX = self.highlightedTile.gridX
                        local swapY = self.highlightedTile.gridY
    
                        local newTileswap = self.board.tiles[tempY][tempX]
    
                        self.highlightedTile.gridX = newTileswap.gridX
                        self.highlightedTile.gridY = newTileswap.gridY
                        newTileswap.gridX = swapX
                        newTileswap.gridY = swapY
    
                        -- swap tiles in the tiles table
                        self.board.tiles[self.highlightedTile.gridY][self.highlightedTile.gridX] =
                            self.highlightedTile
    
                        self.board.tiles[newTileswap.gridY][newTileswap.gridX] = newTileswap
    
                        -- tween coordinates between the two so they swap
                        Timer.tween(0.1, {
                            [self.highlightedTile] = {x = newTileswap.x, y = newTileswap.y},
                            [newTileswap] = {x = self.highlightedTile.x, y = self.highlightedTile.y}
                        })
                        :finish(function()
                            self.highlightedTile = nil
                            self.canInput = true
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            for i = 1, #match, 1 do
                self.score = self.score + 10 * (match[i].variety - 1)
            end
        end
        ...
        keepboard = Board:checkPotentialMatches(self.board.tiles)
        if keepboard == false then
