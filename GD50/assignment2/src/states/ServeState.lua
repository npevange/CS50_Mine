...
function ServeState:enter(params)
    ...
    self.ball0 = Ball(math.random(7), true)
    self.ball1 = Ball(math.random(7), true)
    self.ball2 = Ball(math.random(7), true)
end

function ServeState:update(dt)
    -- have the ball track the player
    self.paddle:update(dt)
    self.ball0.x = self.paddle.x + (self.paddle.width / 2) - 4
    self.ball0.y = self.paddle.y - 8
    self.ball1.x = self.paddle.x + (self.paddle.width / 2) - 4
    self.ball1.y = self.paddle.y - 16
    self.ball2.x = self.paddle.x + (self.paddle.width / 2) - 4
    self.ball2.y = self.paddle.y - 24
